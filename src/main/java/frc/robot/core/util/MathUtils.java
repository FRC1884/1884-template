...
/**
 * A util class containing a multitude of ramp and deadzone functions, primarily for use with controllers.
 * <br><br>
 * Contains quite a few customizable options here to cater to the drivers' preferences for joystick control.
 */
public class MathUtils {
  ...
  public enum TransformPresets {
    LINEAR
            ((value, deadband) ->
                    (abs(value) > deadband) ? (value + deadband * Math.signum(value)) / (1.0 - deadband) : 0.0
            ),
    TRANSFORM
            ((value, jTF) ->
                    (1.0 / (jTF - 1)) * signum(value) * (Math.pow(jTF, abs(value)) - 1)
            ),
    SQUARE
            ((value, none) ->
                    Math.copySign(Math.pow(deadBand.on(value, DEADZONE), 2), value
                    )
            ),
    CUBE
            ((value, weight) ->
  ...
  /**
   * Kills the signal if it is less than the deadband. This is auto-integrated into every other function.
   *
   * @param value The value to be modified.
   * @param deadband The deadband.
   * @return The modified value.
   */
  public static TransformFunction deadBand = (value, deadband) ->
          (abs(value) > deadband) ? (value + deadband * Math.signum(value)) / (1.0 - deadband) : 0.0;
}